...
public class EmojiLikeView extends RelativeLayout
{
    ...
    public void configure (com.opozeeApp.emojilike.EmojiConfig config)
    {
        this.config=config;
        ...
        if (config.backgroundImage!=0)
        {
            ...
            emojiBackgroundView=new GifImageView(getContext());
        ...
        for (int i=0; i<config.emojis.size(); i++)
        {
            ...
            GifImageView emojiImage=new GifImageView(getContext());
...
//            vibe.vibrate(20);
    ...
    private void setUnselectedEmoji(int index)
    {
        if (index>=0&&index<emojiImageViews.size())
        {
            GifImageView view = emojiImageViews.get(index);
            float w=getWeight (view);
            shrinkView(view, index, w, 1f, -config.emojiAnimationSpeed, false);
        }
    }

    private void growView (GifImageView view, int index, float initWeight, float maxWeight, float step, boolean forSelected)
    {
